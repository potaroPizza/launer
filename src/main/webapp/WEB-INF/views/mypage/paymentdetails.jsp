...
<t:wrapper>
	...
	<script type="text/javascript"
		src="<c:url value='/js/paymentdetails.js'/>"></script>
			...
			<ul class="paymentdetails_table">
				...
				<c:if test="${empty list }">
					<li class="paymentdetails_table-row">
						...
						<div class="paymentdetails_col-2" data-label="상품명">결제 내역이
							없습니다.</div>
				...
				<c:if test="${!empty list }">
					<c:forEach var="vo" items="${list }">
						<li class="paymentdetails_table-row"><c:set var="title"
								value="" /> <c:set var="titleCnt" value="0" /> <c:forEach
								var="titleMap" items="${vo.paymentDetails}">
							...
							<div class="paymentdetails_col-2" data-label="상품명">
								...
								<c:if test="${fn: length(title)<=8}">
									<a
										href="/launer/mypage/detailedPaymentHistory?orderNo=${vo.paymentViewVO.orderNo}">${title}</a>
								</c:if>
							</div>
